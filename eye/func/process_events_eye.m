...
events = get_random_events( events, N_rand, [results.overtake.t(1) results.overtake.t(end)], ...
                            idx_baseline );
T = zeros(N_rand,N_event,length(results.overtake.t));
for i = 1 : N_rand
    T(i,:,:) = get_tlocked(pdz, t_pd, events(:,i), params.events.overtake);
end
results.overtake.tlocked_bl2 = squeeze(nanmean(T,1));
...
% Get corresponding random time-locked events for statistical comparison
N_event = length(events);
events = get_random_events( events, N_rand, [results.left_change.t(1) results.left_change.t(end)], ...
                            idx_baseline );
T = zeros(N_rand,N_event,length(results.left_change.t));
for i = 1 : N_rand
    T(i,:,:) = get_tlocked(pdz, t_pd, events(:,i), params.events.left_change);
end
results.left_change.tlocked_bl2 = squeeze(nanmean(T,1));
...
% Get corresponding random time-locked events for statistical comparison
N_event = length(events);
events = get_random_events( events, N_rand, [results.right_change.t(1) results.right_change.t(end)], ...
                            idx_baseline );
T = zeros(N_rand,N_event,length(results.right_change.t));
for i = 1 : N_rand
    T(i,:,:) = get_tlocked(pdz, t_pd, events(:,i), params.events.right_change);
end
results.right_change.tlocked_bl2 = squeeze(nanmean(T,1));
