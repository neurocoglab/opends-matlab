...
function [allsac,allfix] = detecteyemovements2(X,Fs,left_eye_xy,right_eye_xy,badeye,vfac,mindur,degperpixel,smooth,globalthresh,clusterdist,clustermode,plotfig)
...
nsample   = size(X,1);
nepochs   = 1; %size(EEG.data,3);
badepochs = 0; %zeros(nepochs,1);
nbadsmp   = 0;
...
%     end    
    ...
    switch clustermode    
        ...
        case 2
            fprintf('\n\tSaccades separated by fixations < %i ms are clustered.',clusterdist*(1000/Fs));
...
%         l = EEG.data([left_eye_xy(1) left_eye_xy(2)],:,e)';
        l = X(:,1:2);
        vl = vecvel(l,Fs,smoothlevel);
        [l_msdx, l_msdy] = velthresh(vl);
    end
    if rdata
%         r = EEG.data([right_eye_xy(1) right_eye_xy(2)],:,e)';
        r = X(:,3:4);
        vr = vecvel(r,Fs,smoothlevel);
        [r_msdx, r_msdy] = velthresh(vr);
    end
% end
...
%         l = EEG.data([left_eye_xy(1) left_eye_xy(2)],:,e)';
        ...
        badsmp = sum(sum(l<=0)); if badsmp > 0, badepochs = 1; nbadsmp = nbadsmp + badsmp; end
        vl = vecvel(l,Fs,smoothlevel); % get eye velocities
        ...
        if globalthresh % use precomputed velocity thresholds (mean of all epochs)
            ...
            sacL = microsacc_plugin(l,vl,vfac,mindur,l_msdx,l_msdy);        
        end
    end
...
%         r = EEG.data([right_eye_xy(1) right_eye_xy(2)],:,e)';
        r = X(:,3:4)';
        % bad/missing samples in eye track?
        badsmp = sum(sum(r<=0)); if badsmp > 0, badepochs = 1; nbadsmp = nbadsmp + badsmp; end        
        vr = vecvel(r,Fs,smoothlevel); % get eye velocities
        % detect monocular saccades
        if globalthresh % use precomputed velocity thresholds (mean of all epochs)
            sacR = microsacc_plugin(r,vr,vfac,mindur,mean(r_msdx),mean(r_msdy));
        else 
            % compute velocity thresholds from this epoch only
            sacR = microsacc_plugin(r,vr,vfac,mindur,r_msdx,r_msdy);             
        end
    end
...
%         BOUNDDIST_MS = 50;
%         BOUNDDIST    = round(BOUNDDIST_MS / (1000/Fs)); 
...
if ~isempty(allsac)
    ...
    fprintf('\n\tMedian duration:   %.2f ms',median(allsac(:,3))*1000/Fs);
    fprintf('\n\tMedian peak veloc. %.2f %s/s',median(allsac(:,5)),metric);
end
if ~isempty(allfix)
    fprintf('\n%i fixations detected:',size(allfix,1));
    fprintf('\n\tMedian duration:   %.2f ms',median(allfix(:,3))*1000/Fs);
...
if plotfig
    if ldata && rdata
        ...
        ploteyemovements(allsac(:,6),allsac(:,5),allsac(:,7),allfix(:,3)*1000/Fs,mean(allfix(:,[4 6]),2),mean(allfix(:,[5 7]),2),metric);
    elseif ldata
        fprintf('\n--------------------------------------------------------------------')
        fprintf('\nPlotting eye movement properties of left eye...')
        ploteyemovements(allsac(:,6),allsac(:,5),allsac(:,7),allfix(:,3)*1000/Fs,allfix(:,4),allfix(:,5),metric);
    else
        fprintf('\n--------------------------------------------------------------------')
        fprintf('\nPlotting eye movement properties of right eye...')
        ploteyemovements(allsac(:,6),allsac(:,5),allsac(:,7),allfix(:,3)*1000/Fs,allfix(:,6),allfix(:,7),metric);
    end
end
