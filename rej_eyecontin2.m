...
function [seq_bad] = rej_eyecontin2(X,minvals,maxvals,windowsize,Fs)
...
ix_bad = [];
lr = [{'Left(x)'},{'Left(y)'},{'Right(x)'},{'Right(y)'}];
for c = 1:size(X,2)
%     [chnid chntxt] = eeg_decodechan(EEG.chanlocs,chns(c)); % get channel names
    
    if checkminima && checkmaxima
        fprintf('\n%s eye : rejecting values < %i or > %i',lr{c},minvals(c),maxvals(c));
        ix = find(X(:,c) < minvals(c) | X(:,c) > maxvals(c));
    elseif checkminima
        fprintf('\n%s eye : rejecting values < %i',lr{c},minvals(c));
        ix = find(X(:,c) > maxvals(c));
    else
        fprintf('\n%s eye : rejecting values > %i',lr{c},maxvals(c));
        ix = find(X(:,c) < minvals(c) );
    end
    ix_bad = [ix_bad ix];
end
ix_bad = unique(ix_bad);
if windowsize > 0
    fprintf('\nRejecting an extra plusminus %i samples (%.0f ms) around each out-of-range interval.',windowsize,windowsize*1000/Fs)
end
