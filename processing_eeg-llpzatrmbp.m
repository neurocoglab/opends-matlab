...
%         clear data_flt;
        ...
        if params.eeg.ica.apply
            ...
            comp = ft_componentanalysis(cfg_ica, data_ica);
            
            save(sprintf('%s/processing_results_eeg_ica.mat',outdir), 'comp');
        ...
        cfg_hb.continuous = 1;
        cfg_hb.demean = 'no';
%         cfg_hb.trl = [1 data.eeg.ft.sampleinfo(2) 1 1];
%         cfg.artfctdef.minaccepttim   = params.eeg.artifacts.minaccepttim_timefreq;
%         cfg_hb = ft_rejectartifact(cfg_hb);
       
        N_bands = size(params.eeg.hilbert.bands,1);
        data_hilbert = cell(N_bands,1);
        ...
        for b = 1 : N_bands
            ...
            fprintf('\nComputing envelopes for [%1.1f to %1.1f Hz]\n', ...
                        params.eeg.hilbert.bands(b,1), params.eeg.hilbert.bands(b,2));
            
            cfg_hb.hpfilter = 'yes';
            cfg_hb.hpfreq = params.eeg.hilbert.bands(b,1);
            cfg_hb.hpfiltord = 2;
            cfg_hb.lpfilter = 'yes';
            cfg_hb.lpfreq = params.eeg.hilbert.bands(b,2);
            cfg_hb.lpfiltord = 10;
            
            data_b = ft_preprocessing(cfg_hb, data_flt);
            
            X = zeros(N_chan, L);
            t = zeros(L,1);
            idxx = 1;
            samples = zeros(length(data_b.trial),2);
            
            for j = 1 : length(data_b.trial)
               tt =  data_b.time{j};
               t(idxx:idxx+length(tt)-1) = tt;
               xx =  data_b.trial{j};
               X(:,idxx:idxx+length(tt)-1) = xx;
               
               samples(j,:) = [idxx idxx + length(tt)-1];
               idxx = idxx + length(tt);
            ...
            for k = 1 : size(up,1)
               [~,pp] = findpeaks(up(k,:));
               idx = [1 pp length(t)];
               up(k,:) = smooth(linterp(t(idx), up(k,idx), t),1000);
            end
            
            for j = 1 : length(data_b.trial)
                data_b.trial(j) = {up(:,samples(j,1):samples(j,2))};
            end
            
            data_hilbert(b) = {data_b};
        ...
        if show_plots
           ...
           plot_params.eeg.line_widths = [0.5 1.5];
           plot_params.eeg.lighten = [true, false];
           plot_params.eeg.height = 20;
           plot_params.eeg.interpolate = true;
           plot_params.eeg.smooth = 100;
           
           for b = 1 : N_bands

%                plot_eeg_blinks(et_results.results, data_hilbert{b}, [{'vEOGover'},{'Fz'},{'FC5'},{'POz'},{'T7'}], 10);
               plot_events2(et_results.results, plot_params, [{'Rounds'},{'LaneChanges'},{'Baseline'},{'Saccades'},{'Overtakes'}], ...
                            [{data_flt}, data_hilbert(b)], [{'vEOGover'},{'Fz'},{'FC5'},{'POz'},{'T7'}]);
               hh = title(sprintf('Hilbert envelopes for [%1.1f to %1.1f Hz]', params.eeg.hilbert.bands(b,1), params.eeg.hilbert.bands(b,2)));
               hh.FontSize = 16;
           end
        end
        
        save(sprintf('%s/processing_results_eeg_hilbert.mat',outdir), 'data_hilbert', '-v7.3');
        clear data_hilbert;
        ...
        data_erp.left_change.all = ft_preprocessing(cfg);
        ...
        cfg.trl = trl(idx,1:4);
        cfg = ft_rejectartifact(cfg);
        data_erp.left_change.easy = ft_preprocessing(cfg);
        ...
        cfg.trl = trl(idx,1:4);
        cfg = ft_rejectartifact(cfg);
        data_erp.left_change.difficult = ft_preprocessing(cfg);
        ...
        cfg = ft_rejectartifact(cfg);
        data_epochs.baseline = ft_preprocessing(cfg);
        ...
        cfg.trl = trl_epochs(idx,1:4);
        cfg = ft_rejectartifact(cfg);
        data_epochs.passing = ft_preprocessing(cfg);
...
%         idx = find(trl_epochs(:,5)==1);
%         cfg.trl = trl_epochs(idx,1:4);
%         mydata = ft_preprocessing(cfg, data_epochs.baseline);
...
%         idx = find(trl_epochs(:,5)==2);
%         cfg.trl = trl_epochs(idx,1:4);
%         mydata = ft_preprocessing(cfg, data_epochs.passing);
%         mydata = ft_rejectartifact(cfg,  mydata);
...
%         clear data_epochs;
        ...
        %% FFT & Time/Freq Analyses - Easy versus Difficult Events
...
%         cfg_timefreq.t_ftimwin  = ones(length(cfg_timefreq.foi),1).*0.5;
        ...
        mydata = ft_preprocessing(cfg);
        mydata = ft_rejectartifact(cfg,  mydata);
        data_timefreq.left_change.all = ft_freqanalysis(cfg_timefreq, mydata);
        ...
        mydata = ft_preprocessing(cfg);
        mydata = ft_rejectartifact(cfg,  mydata);
        data_timefreq.left_change.easy = ft_freqanalysis(cfg_timefreq, mydata);
        ...
        mydata = ft_preprocessing(cfg);
        mydata = ft_rejectartifact(cfg,  mydata);
        data_timefreq.left_change.difficult = ft_freqanalysis(cfg_timefreq, mydata);
...
%         save(sprintf('%s/processing_results_eeg.mat',outdir), 'data', '-v7.3');
