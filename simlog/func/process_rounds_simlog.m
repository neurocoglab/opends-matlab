...
output_dir = sprintf('%s/%s/%s', params.root_dir, params.data_dir, params.output_dir);
...
results.sim2track = sim2track;
    ...
    function Mnew = fix_repeats( matrix )
        ...
        for ii = 1 : size(matrix,1)-1
            
            last_dist = matrix{ii,5};
            last_cycle = matrix{ii,3};
            this_dist = matrix{ii+1,5};
            this_cycle = matrix{ii+1,3};
        ...
        for ii = 2 : size(matrix,1)
            ...
            last_dist = matrix{ii-1,5};
            last_cycle = matrix{ii-1,3};
            this_dist = matrix{ii,5};
            this_cycle = matrix{ii,3};
