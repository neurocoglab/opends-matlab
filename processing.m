...
for i = 1 : length(subjects)
    ...
    %     summary.zscore.passing.pupil = [summary.zscore.passing.pupil;results.epochs.zscore.passing.pupil];
        ...
        fprintf('Error processing epochs for subject %s\n', subject);
    end
...
if params.epochs.save_plots
    params.epochs.plot_scatter=true;
...
if show_plots
    ...
    load( summary_file );
    params.epochs.plot_scatter=true;
    h = plot_epoch_summary( summary, params );
...
% Do summary stats
summary.stats = analyze_events( summary );

save( sprintf('%s/%s', params.output_dir, params.events.summary_output), 'summary', '-v7.3' );
fprintf('Done events processing.\n');

if params.events.save_plots
    plot_event_stats2( summary, params, true );
end
